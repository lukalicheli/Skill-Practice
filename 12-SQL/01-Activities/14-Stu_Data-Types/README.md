...
* It's done when the `id` column only accepts integers and cannot be `NULL`.

* It's done when the `first_name` and `last_name` columns only accept text with a maximum size of 30 and cannot be `NULL`.

* It's done when the `value_card_member` column only accepts `true` or `false` and cannot be `NULL`.
