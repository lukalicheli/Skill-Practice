...
* It is done when I have registered a new Workbox service worker inside the `src/index.js` file using the `InjectManifest` constructor. (Set the `swDest` option to output service worker to `service-worker.js`.)
